...
void	fill_offsets(t_offsets *offs, t_file_info **info, int len)
{
	...
	while (++i < len)
	{
		if (ndigits(info[i]->nlinks) + 1 > offs->n_links)
			offs->n_links = ndigits(info[i]->nlinks) + 1;
		if (ndigits(info[i]->size) + 1 > offs->size)
			offs->size = ndigits(info[i]->size) + 1;
		...
		{
			if (ndigits(info[i]->major) + 1 > offs->major)
				offs->major = ndigits(info[i]->major) + 1;
			if (ndigits(info[i]->minor) + 1 > offs->minor)
				offs->minor = ndigits(info[i]->minor) + 1;
		}
	}
